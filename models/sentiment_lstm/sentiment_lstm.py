...
def create_embedding_layer(word_index, embeddings_path):
    ...
    vocabulary_size = len(embeddings.key_to_index)
...
def get_model_without_embeddings(model, input_shape):
    ...
    model.summary()
    x = Dropout(0.4)(input)
    x = LSTM(128, weights=model.get_layer('lstm').get_weights())(x)
    x = Dense(64, activation='relu', weights=model.get_layer('dense').get_weights())(x)
    x = Dropout(0.5)(x)
    y = Dense(1, activation='sigmoid', weights=model.get_layer('dense_1').get_weights())(x)
